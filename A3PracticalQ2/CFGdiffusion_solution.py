...
def p_sample(model, x, t, t_index, coefficients, y=None, guidance_scale=1.0, noise=None):
    ...
    # Again, need everything at time t
...
def p_sample_loop(model, shape, timesteps, T, coefficients, y=None, guidance_scale=1.0, noise=None):
    ...
    for i in tqdm(
        ...
        if noise is None:
            img = p_sample(model, img, t, i, coefficients, y, guidance_scale, noise=None)
        else:
            img = p_sample(model, img, t, i, coefficients, y, guidance_scale, noise=noise[i])
        imgs.append(img.cpu())
...
def p_losses(denoise_model, x_start, t, coefficients, y=None, p_uncond=0.1, noise=None, loss_type="l2"):
    ...
    x_noisy = q_sample(x_start, t, coefficients, noise=noise)
    ...
    if y is None:
        ...
        #reshape mask to (B, 1, 1, 1) for broadcasting
        mask_reshaped = mask[:, None, None, None].float()
        eps_theta = mask_reshaped * eps_uncond + (1 - mask_reshaped) * eps_cond
