...
def q_sample(x_start, t, coefficients, noise=None):
    ...
    sqrt_one_minus_alphas_t = extract(
        sqrt_one_minus_alphas_cumprod, t, x_start.shape
    )
    x_noisy = sqrt_alphas_t * x_start + sqrt_one_minus_alphas_t * noise
...
def p_sample(model, x, t, t_index, coefficients, noise=None):
    ...
    with torch.no_grad():
        ...
        # need beta, sqrt_one_minus_alphas_cumprod, sqrt_recip_alphas, posterior_variance at time t
        ...
        # theta predicted noise
        epsilon_theta = model(x, t)
        # Compute p_mean
        p_mean = sqrt_recip_alphas_t * ( x - betas_t / sqrt_one_minus_alphas_cumprod_t * epsilon_theta)

        if t_index == 0:
            sample = p_mean
        else:
            # noise uses variance term here
            sample = p_mean + torch.sqrt(posterior_variance_t) * (torch.randn_like(x) if noise is None else noise)
...
def p_sample_loop(model, shape, timesteps, T, coefficients, noise=None):
    ...
    with torch.no_grad():
        b = shape[0]
        # Start from pure noise (x_T) and then go to 0?
        ...
        for i in tqdm(
            ...
            # always need current reverse-diffusion timestep to tell how noisy image is
...
def p_losses(denoise_model, x_start, t, coefficients, noise=None):
    ...
    # need to verify tuple size: sometimes 2, sometimes 4, here it doesn't say.
    # write 2 cases: len =2 and len=4
    if len(coefficients) > 2:
        q_coeffs = (coefficients[4], coefficients[5])
    else:
        q_coeffs = coefficients
    
    # forward diffusion x_t | x_0
    x_noisy = q_sample(x_start, t, q_coeffs, noise=noise)

    # backward prediction epsilon_theta at x_t, t
